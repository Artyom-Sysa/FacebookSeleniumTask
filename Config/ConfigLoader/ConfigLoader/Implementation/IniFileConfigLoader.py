        ...
        try:
            ...
            for section in cfg_parser.sections():
                ...
                for key in cfg_parser[section]:
                    is_n, n_value = Utils.is_bool(cfg_parser.get(section, key))
                    is_b, b_value = Utils.is_number(cfg_parser.get(section, key))

                    if is_n:
                        self.configuration.settings[section][key] = n_value
                    elif is_b:
                        self.configuration.settings[section][key] = b_value
                    else:
                        self.configuration.settings[section][key] = cfg_parser.get(section, key)

                    Logger.add_to_journal(__file__, Level.DEBUG, 'Parameter {} set to value {}'.format(key, cfg_parser.get(section, key)))
